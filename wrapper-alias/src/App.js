...
const TARGET_URL = "https://meetqonline.com/meetqwords/?from_app=1";
// const TARGET_URL = "https://www.your-live-website.com/";
const REDIRECT_URL = "https://meetqonline.com//meetqwords/";
...
useEffect(() => {
  const handleMessage = (event) => {
    if (event.data?.type === 'EXIT_APP') {
      CapacitorApp.exitApp();
    }

    if (event.data?.type === 'OPEN_EXTERNAL' && event.data.url) {
      Browser.open({ url: event.data.url });
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.text}>We couldn't connect to the server.<br />Please make sure your server is running and your device is connected to the same network.</p>
